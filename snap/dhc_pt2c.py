...
def get_num_elem_per_mode(Graph):
    ...
    first_node = modeNet.BegMMNI()
    name = snap.TStrV()
	#name = snap.TStrTAttrPr("name", snap.atStr)
    test = GetEdgeI(0)
    for i in range(1,30):
        try:
            print test.GetId()
