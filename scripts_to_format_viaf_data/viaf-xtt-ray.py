...
if __name__ == "__main__":
    ...
    with open(infile, 'r+b') as read_handle:
        with open(outfile, 'w') as write_handle:
            ...
            while more_vals or remaining_tasks:

                    if len(remaining_tasks) < 16 and more_vals == True:
                        try:
                            remaining_tasks.append(xmlToTsv.remote(next(iterator)))
                        except:
                            more_vals == False

                    ready_ids, remaining_tasks = ray.wait(remaining_tasks, timeout=0.00000001)
                    for val in ready_ids:
                        count += step_size
                        print("{} results in {}".format(count, time.time() - start_time))
                        csv_writer.writerows(ray.get(val))
