...
if [ $# -eq 4 ];then
    neo4j-admin import --mode=csv --database "$1" --id-type string \
                        ...
                        --nodes "headers/language_header.csv,data/language_batch.csv" \
                        ...
                        --relationships "headers/language_rel_header.csv,data/language_rel_batch.csv" \
                        --ignore-duplicate-nodes true --delimiter "$2" --high-io true --quote "$3" --array-delimiter "$4"

else
    echo "Usage: ./importer.sh dbname delimter quotechar array-delimiter"
fi
